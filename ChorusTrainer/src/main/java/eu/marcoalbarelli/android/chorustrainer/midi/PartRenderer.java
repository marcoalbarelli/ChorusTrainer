...
public class PartRenderer {
    ...
    public PartRenderer(Context context, MidiFile source ){
        this.context = context;
        this.source = source;
    }


    private MidiFile removeTrackFromFile(MidiFile mf, int trackId){

        return mf;
    }
    private MidiFile emphasizeTrackInFile(MidiFile mf, int trackId){
    ...
    private MidiFile reduceTempoInFile(MidiFile mf, int newTempo){
    ...
    private MidiFile manipulateTracks(MidiFile mf){
    ...
    private void saveModifiedFile(MidiFile mf) {
        try {
            File output = new File(Constants.OUTPUT_TMP_FILE_NAME);
            mf.writeToFile(output);
        } catch(IOException e) {
